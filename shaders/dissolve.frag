...
// Identical to triangle.frag but with noise-based pixel discard and HDR edge glow.
// Used for champion death/spawn dissolve animations.

layout(set = 1, binding = 0) uniform sampler2D textures[4096]; // global bindless array

// Push constants for dissolve parameters (shared layout with skinned.vert)
layout(push_constant) uniform DissolvePC {
    uint  boneBaseIndex;     // consumed by skinned.vert, ignored here
    float dissolveProgress;  // 0.0 = fully visible, 1.0 = fully dissolved
    float edgeWidth;         // glow border width (world-space fraction)
    uint  noiseTexIdx;       // bindless index of the noise texture
    vec4  edgeColor;         // xyz = HDR edge glow color
} dissolve;
...
void main() {
    // ── Dissolve: noise-based pixel clipping ─────────────────────────────────
    float noise = texture(textures[nonuniformEXT(dissolve.noiseTexIdx)], fragTexCoord).r;
    if (noise < dissolve.dissolveProgress) discard;

    // Edge glow factor: pixels close to the dissolve threshold get a hot edge
    float edgeFactor = 1.0 - smoothstep(dissolve.dissolveProgress,
                                         dissolve.dissolveProgress + dissolve.edgeWidth,
                                         noise);
