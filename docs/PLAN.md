...
- **Clearcoat specular layer**: Second GGX specular lobe in triangle.frag for high-metallic, low-roughness surfaces (metallic>0.5, roughness<0.3). Ultra-smooth top coat (roughness=0.08) with dielectric F0=0.04, modulated by material params. Creates car paint / lacquered look with sharp highlight atop diffuse base coat.
- **Pyramid mesh**: `Model::createPyramid(baseSize, height)` generates a 4-sided pyramid with per-face flat normals. 4 side triangles + 2 bottom triangles = 18 verts, 18 indices. Clean UV mapping for texture application. Added as a sandstone pyramid with noise texture.
- **Procedural noise texture**: `Texture::createNoise()` generates a 256×256 greyscale FBM noise. Hash-based value noise with Hermite interpolation, 4 octaves at 1/32 pixel frequency. Produces organic turbulence pattern suitable for stone, cloud, or distortion effects. Applied to pyramid mesh.
